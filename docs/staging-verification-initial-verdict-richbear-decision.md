...
- **功能驗收基準**：**633d215**。正式站已確認跑此版（deploy logs 顯示 `[build-version] commit=633d215`；/api/version 為 404 表示未部署 c903a5e）。不再以 /api/version 為前提，**直接以 633d215 為功能驗收基準**，依下方清單跑 A/B/C。貼回 staging 真實操作結果後，再填寫「哪塊不像華麗熊」「哪塊最卡」「下一輪只修哪兩點」。

---

## 二、最終 Staging 驗收清單（A/B/C）
...
## 三、Staging 真實操作結果（請貼回）

（請將 A/B/C 實際操作結果貼於此，例如：每項通過/部分通過/失敗、關鍵畫面或 API 回應、失敗步驟與根因。貼回後再填寫 §四 三項判斷。）

---

## 四、做完 A/B/C 後的判斷（貼回結果後填寫）
...
**3. 下一輪只修哪兩點**  
- 依「最影響實用性」與「最影響可信度」從本輪結果選兩點，且只列兩點。必須對應到本輪 A/B/C 與上述 1、2 的結論。
...
- **功能驗收基準**：633d215（初審落地、靈感池華麗熊、第一次決策點寫回）。
