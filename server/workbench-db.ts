...
/** Phase 3: 批次建立 workbench tasks，單一 transaction，避免 N+1。 */
export async function createWorkbenchTasksBatch(
  inputs: Array<Omit<WorkbenchTask, "id" | "createdAt" | "updatedAt">>
): Promise<WorkbenchTask[]> {
  if (inputs.length === 0) return [];
  const tasks = await prisma.$transaction(async (tx) => {
    const created: Awaited<ReturnType<typeof prisma.workbenchTask.create>>[] = [];
    for (const input of inputs) {
      const t = await tx.workbenchTask.create({
        data: {
          productName: input.productName ?? null,
          creativeId: input.creativeId ?? null,
          draftId: input.draftId ?? null,
      ...
      await tx.workbenchAudit.create({
        data: {
          userId: input.createdBy,
          entityType: "task",
          entityId: t.id,
  ...
  return tasks.map((r) => rowToTask(r));
}
