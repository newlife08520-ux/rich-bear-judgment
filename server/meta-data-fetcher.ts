...
export async function fetchMetaCampaignData(
  ...
  try {
    ...
    const [campaignsRes, insightsRes, prevInsightsRes, adsetsRes] = await Promise.all([
      fetch(
        `https://graph.facebook.com/v19.0/${actId}/campaigns?fields=id,name,status,objective,effective_status,updated_time&limit=100&access_token=${encodeURIComponent(token)}`
      ),
      ...
      fetch(
        `https://graph.facebook.com/v19.0/${actId}/adsets?fields=campaign_id,daily_budget,bid_strategy,bid_amount,optimization_goal,effective_status&limit=500&access_token=${encodeURIComponent(token)}`
      ),
    ]);

    const [campaignsData, insightsData, prevInsightsData, adsetsData] = await Promise.all([
      campaignsRes.json(),
      insightsRes.json(),
    ...
    >();
    if (adsetsRes.ok && adsetsData.data) {
      for (const row of adsetsData.data as {
        campaign_id?: string;
        bid_strategy?: string;
        daily_budget?: string;
        bid_amount?: string;
        ...
        if (!cid || adsetByCampaign.has(cid)) continue;
    ...
    const results: CampaignMetrics[] = campaigns.map((c: any) => {
      ...
      const adsetRow = adsetByCampaign.get(c.id);
      const biddingType = adsetRow?.bid_strategy?.trim() ? adsetRow.bid_strategy : undefined;
      let spendFullness: number | undefined;
      if (adsetRow?.daily_budget != null && adsetRow.daily_budget !== "") {
        const dailyMinor = parseFloat(String(adsetRow.daily_budget));
        if (Number.isFinite(dailyMinor) && dailyMinor > 0) {
          const dailyMajor = dailyMinor / 100;
          const denom = dailyMajor * rangeDays;
          if (denom > 0) spendFullness = Math.min(1.5, spend / denom);
        }
      }
      /** 7.1：以 bid_amount（minor）粗映射為「目標成果數值」提示；缺欄位維持 null */
      let targetOutcomeValue: number | null = null;
      if (adsetRow?.bid_amount != null && String(adsetRow.bid_amount).trim() !== "") {
        const ba = parseFloat(String(adsetRow.bid_amount));
        if (Number.isFinite(ba) && ba > 0) {
          targetOutcomeValue = ba / 100;
        }
      }

      let dailyBudgetMinor: number | null = null;
      if (adsetRow?.daily_budget != null && String(adsetRow.daily_budget).trim() !== "") {
        const db = parseFloat(String(adsetRow.daily_budget));
        if (Number.isFinite(db) && db >= 0) {
          dailyBudgetMinor = Math.round(db);
        }
      }
      const metaEffectiveStatus =
        typeof c.effective_status === "string" && c.effective_status.trim() ? c.effective_status.trim() : null;
      const metaUpdatedAt =
        typeof c.updated_time === "string" && c.updated_time.trim() ? c.updated_time.trim() : null;
      const metaAdSetEffectiveStatus =
        typeof adsetRow?.effective_status === "string" && adsetRow.effective_status.trim()
          ? adsetRow.effective_status.trim()
          : null;
      ...
      return {
        ...
        targetOutcomeValue,
        spendFullness: spendFullness ?? null,
        dailyBudgetMinor,
        metaEffectiveStatus,
        metaUpdatedAt,
