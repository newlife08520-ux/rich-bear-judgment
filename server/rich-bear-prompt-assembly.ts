...
/** Phase 5：護欄 — 視角 Overlay 與 Data Context 字數上限，避免 token 爆量。 */
export const MAX_CUSTOM_MAIN_PROMPT_CHARS = 8000;
export const MAX_DATA_CONTEXT_CHARS = 16000;

function truncateWithSuffix(s: string, max: number, suffix = "…（已截斷）"): string {
...
export function getAssembledSystemPrompt(options: AssembleOptions): string {
  ...
  const publishedOverlay = truncateWithSuffix((customMainPrompt && customMainPrompt.trim()) || "", MAX_CUSTOM_MAIN_PROMPT_CHARS);
  const layer2Calibration = getHiddenCalibration();
  const layer4Data = truncateWithSuffix((dataContext && dataContext.trim()) || "", MAX_DATA_CONTEXT_CHARS);
