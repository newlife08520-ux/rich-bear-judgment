...
/** @legacy-batch6_8 歷史報表轉換層；與新版 Product／Creative 敘事並存。變更請對照 verify:batch25。 */
...
export function buildRealBudgetRecommendations(metrics: CampaignMetrics[]): FbBudgetRecommendation[] {
  ...
  for (const m of scalable.slice(0, 3)) {
    ...
    recs.push({
      ...
      suggestedBudgetDaily: Math.round(m.spend + amtStep),
  ...
  for (const m of fatigued.slice(0, 2)) {
    ...
    recs.push({
      ...
      campaignId: m.campaignId,
      suggestedBudgetDaily: Math.round(m.spend - reduceAmt),
