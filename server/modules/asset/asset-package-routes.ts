...
/** POST /api/asset-packages/:id/versions/upload - 上傳檔案，回傳 { fileUrl, fileName, fileType }（權限：須為該素材包擁有者） */
assetPackageRouter.post("/:id/versions/upload", (req: Request, res: Response) => {
  upload(req, res, (err) => {
    try {
      ...
      const result = uploadStorage.saveFile(userId, file.originalname, file.mimetype || "", file.buffer);
      // 真實偵測尺寸與比例：圖片用 image-size 讀取寬高，影片用 ffprobe；結果由前端建立版本時寫入 DB（aspectRatio, detectStatus, detectSource, detectedWidth/Height）
      const detection = detectMedia(file.buffer, file.mimetype || "", file.originalname);
      const mime = (file.mimetype || "").toLowerCase();
      let thumbnailUrl: string | undefined;
      if (mime.startsWith("image/")) {
        thumbnailUrl = result.fileUrl;
      } else if (mime.startsWith("video/")) {
        thumbnailUrl = generateVideoThumbnail(userId, file.buffer, file.mimetype || "", file.originalname) ?? undefined;
      }
