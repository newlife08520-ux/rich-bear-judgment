...
publishRouter.get("/drafts", (req: Request, res: Response) => {
  try {
    const userId = getUserId(req);
    const list = publishService.listDrafts(userId);
    res.json(list);
  } catch (e) {
...
publishRouter.post("/drafts", (req: Request, res: Response) => {
  try {
    const userId = getUserId(req);
    const result = publishService.createDraft(userId, req.body);
    if (!result.ok) {
...
publishRouter.post("/drafts/batch", (req: Request, res: Response) => {
  try {
    const userId = getUserId(req);
    const { batchId, drafts } = req.body as { batchId?: string; drafts?: unknown[] };
    const result = publishService.createDraftBatch(userId, { batchId: batchId ?? "", drafts: drafts ?? [] });
    if (!result.ok) {
      const status = result.notFound ? 404 : 400;
      const body: { message: string; errors?: unknown } = { message: result.message };
      if (status === 400 && result.errors) body.errors = result.errors;
...
publishRouter.get("/logs", (req: Request, res: Response) => {
  try {
    const userId = getUserId(req);
    const list = publishService.listLogs(userId);
    res.json(list);
  } catch (e) {
    res.status(401).json({ message: "未登入" });
  }
...
publishRouter.get("/drafts/:id", (req: Request, res: Response) => {
  try {
    const userId = getUserId(req);
    const id = getParamId(req, "id");
    const draft = publishService.getDraft(userId, id);
    if (!draft) {
...
publishRouter.put("/drafts/:id", (req: Request, res: Response) => {
  try {
    const userId = getUserId(req);
    const id = getParamId(req, "id");
    const result = publishService.updateDraft(userId, id, req.body);
    if (!result.ok) {
      const status = result.notFound ? 404 : 400;
      const body: { message: string; errors?: unknown } = { message: result.message };
      if (status === 400 && result.errors) body.errors = result.errors;
