...
import { resumeCampaign } from "../../meta-execution/meta-execution-client";
import { storage } from "../../../storage";

const ACTION_TYPE = "meta_campaign_resume";

function validate(payload: unknown): asserts payload is Record<string, unknown> {
  validateCampaignId(payload);
}

export const metaCampaignResumeHandler: IExecutionHandler = {
  ...
  buildPreview(payload: Record<string, unknown>, _ctx: ExecutionContext): PreviewResult {
    ...
    return {
      summary: `啟動 Meta 廣告活動：${id}…`,
      steps: [
        "1. 驗證 campaignId",
        "2. 呼叫 Meta Graph API 將 status 設為 ACTIVE",
        "3. 回傳結果與 rollback 快照",
      ],
  ...
  async apply(payload: Record<string, unknown>, ctx: ExecutionContext): Promise<ApplyResult> {
    ...
    const result = await resumeCampaign(token, campaignId);
    if (!result.success) {
      throw new Error(result.message ?? "resume 失敗");
    }
    return {
      resultSummary: result.message ?? "已啟動",
