...
function deriveProduct(p: ProductLevelMetrics, config: ThresholdConfig | null): DerivedProduct {
  ...
  if (p.spend >= t.spendThresholdStop && p.roas < t.roasTargetMin) {
    ...
    ruleTags.push("危險");
    if (productStatus !== "stop") productStatus = "danger";
  }
...
export function buildDecisionCards(input: DecisionCardsInput, thresholdConfig?: ThresholdConfig | null): DecisionCardBlock[] {
  ...
  const lowRoasCreatives = creativeLeaderboard.filter((c) => c.spend >= 200 && c.roas < ROAS_TARGET_MIN);
  ...
  const cards: DecisionCardBlock[] = [
    ...
    },
    {
      key: "owner",
      label: "owner 建議",
      conclusion: "請至「商品作戰室」為高花費商品與素材指派商品 owner、投手 owner、素材 owner；未指派任務請在「任務中心」分配。",
      triggerRule: "依花費與狀態優先指派",
      evidenceMetrics: `商品數 ${products.length}、高花費（>1000）${products.filter((p) => p.spend > 1000).length} 個`,
      suggestedAction: "在商品作戰室設定三種 owner，並在任務中心指派待辦",
      impactAmount: "—",
      confidence: "medium",
    },
    {
