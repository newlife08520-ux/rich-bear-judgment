...
export const PERCENT_KEYS = ["funnelAtcTolerance", "funnelPurchaseTolerance"] as const;
...
export const PRESETS: { id: string; label: string; description: string; config: Record<string, number> }[] = [
  {
    id: "conservative",
    label: "保守測試",
...
export function formatDisplayValue(key: string, val: number, def: FieldDef): string {
  if (def.isPercent) return toPercentDisplay(val);
  if (def.isRatio || key === CTR_KEY) return `${val}%`;
  return String(val);
}
