...
function storageKey(userId: string) {
  return `app-scope:${userId}`;
}

function loadScope(userId: string): AppScope {
  try {
...
function persistScope(scope: AppScope, userId: string) {
  try {
    localStorage.setItem(storageKey(userId), JSON.stringify(scope));
  } catch {}
}

function useAppScopeInternal(userId: string) {
  const [scope, setScope] = useState<AppScope>(() => loadScope(userId));
  ...
  const scopeKey = useMemo(
    () =>
      buildScopeKey(
        ...
        scope.datePreset,
        scope.datePreset === "custom" ? scope.customStart : undefined,
        scope.datePreset === "custom" ? scope.customEnd : undefined
      ),
    [
      userId,
      scope.selectedAccountIds,
...
export function AppScopeProvider({ userId, children }: { userId?: string; children: React.ReactNode }) {
  const value = useAppScopeInternal(userId ?? "1");
